...
pmap((year, gas) for 
    ...
    results = 
        MimiGIVE.compute_scc(m, 
                            ...
                            pulse_size              = 1.0,                      ## scales the defalut pulse size of 1Gt to 100k metric tons 
    ...
    ## blank data
    schfcs = DataFrame(sector = String[], discount_rate = String[], trial = Int[], schfc = Int[]);
        
    ## populate data
    for (k, v) in results[:scc]
        for (i, sc) in enumerate(v.ce_sccs)
            push!(schfcs, (sector = String(k.sector), discount_rate = k.dr_label, trial = i, schfc = round(Int, sc*pricelevel_2005_to_2020)))
        end
    end
    ...
    schfcs_mean = combine(groupby(schfcs, [:sector, :discount_rate]), :schfc => (x -> round(Int, mean(x))) .=> :schfc)
